...
# display images generated from randomly sampled latent vector 15*15 images
...
for i in range(n):
    for j in range(n):
        z_sample = np.array([np.random.uniform(-1,2 ,size=latent_dim)])
        x_decoded = generator.predict(z_sample)
        img_new = x_decoded.shape[1]
        digit = x_decoded[0].reshape(img_new, img_new,3)
        plt.imshow(digit)
        plt.savefig('result2/'+str(i)+'_'+str(j)+'.png')


# z_sample = np.array([np.random.normal([0,0,0],[1,1,1],1)])
#         z_sample = np.array([np.random.normal([0,0,0],[1,1,1],1)])
#         ep = (np.random.choice([z_sample,z_sample2],p=[0.3,0.7]))